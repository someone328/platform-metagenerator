...
public class DomainGeneratorVerticle extends AbstractVerticle {
  ...
  public void start() {
    try {
      ...
      generateDomain(GitCloneWrapper.calculateFolderNameFrom(gitUrlFrom), outputGitFolderName);

      wait4GenerationEndAndStopVerticle(gitUrlTo, outputGitFolderName);
    } catch (Exception e) {
      e.printStackTrace();
  ...
  private void wait4GenerationEndAndStopVerticle(String gitUrlTo, String outputGitFolderName) {
    ...
    Flowable<Long> ticker = Flowable.interval(200, TimeUnit.MILLISECONDS, scheduler);
    ticker
        .map(
            tickId ->
                vertx
  ...
  private void pushGeneratedAndStopCurrentVerticle(String gitUrlTo) {
    ...
    vertx.setTimer(500, handler -> vertx.undeploy(deploymentID()));
  }
